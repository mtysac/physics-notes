...
function drawGrid() {
    ...
    ctx.strokeStyle = 'rgba(186,186,191,0.07)';
    ctx.lineWidth = 1;
...
function drawGhostTrajectory() {
    ctx.beginPath();
    ctx.strokeStyle = 'rgba(170,170,173,0.18)';
...
function drawAngleArc() {
    ...
    ctx.beginPath();
    ctx.strokeStyle = 'rgba(186,186,191,0.4)'; ctx.lineWidth = 1;
    ctx.arc(ox, oy, 28, -Math.PI, -Math.PI + angleRad, false);
    ctx.stroke();
    ctx.fillStyle = 'rgba(186,186,191,0.5)'; ctx.font = '10px Poppins';
    ctx.fillText(Math.round(angleRad * 180 / Math.PI) + '°', ox + 32, oy - 10);
    ctx.beginPath();
    ctx.strokeStyle = 'rgba(186,186,191,0.3)'; ctx.lineWidth = 1.5;
    ctx.moveTo(ox, oy);
    ctx.lineTo(ox + 40 * Math.cos(angleRad), oy - 40 * Math.sin(angleRad));
    ctx.stroke();
    ctx.beginPath(); ctx.fillStyle = 'rgb(186,186,191)';
...
function drawMaxHeightLine() {
    ...
    const W = canvas.width;
    ctx.strokeStyle = 'rgba(186,186,191,0.2)'; ctx.lineWidth = 1;
    ctx.setLineDash([6, 8]);
    ctx.beginPath(); ctx.moveTo(0, sy); ctx.lineTo(W, sy); ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillStyle = 'rgba(186,186,191,0.3)'; ctx.font = '10px Poppins';
...
function drawStickman(cx, cy) {
    const r = 7;
    ctx.strokeStyle = 'rgba(59,68,83,0.9)'; ctx.lineWidth = 1.2;
    ctx.beginPath(); ctx.arc(cx, cy - r - 5, 3, 0, Math.PI * 2); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx, cy - r - 2); ctx.lineTo(cx, cy - r + 5); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx - 4, cy - r + 1); ctx.lineTo(cx + 4, cy - r + 1); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx, cy - r + 5); ctx.lineTo(cx - 3, cy - r + 10); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx, cy - r + 5); ctx.lineTo(cx + 3, cy - r + 10); ctx.stroke();
}
...
function animate() {
    ...
    if (py <= 0 && t > 0.05) {
        drawFrame(px, 0, pvx, 0);
        const [lx, ly] = toScreen(px, 0);
        ctx.strokeStyle = 'rgb(170,170,173)'; ctx.lineWidth = 2;
        ...
        ctx.fillStyle = 'rgb(186,186,191)'; ctx.font = '11px Poppins';
...
canvas.addEventListener('click', e => {
    ...
    const [ox, oy] = toScreen(0, h0);
    const dx = e.clientX - rect.left - ox * (rect.width / canvas.width);
    const dy = oy * (rect.height / canvas.height) - (e.clientY - rect.top);
    if (dx <= 0) return;
    const deg = Math.round(Math.atan2(dy, dx) * 180 / Math.PI);
    if (deg < 1 || deg > 89) return;
